...
El speedup máximo que se puede lograr depende de la fracción serial
$(1-p)$ de la tarea, según la Ley de Amdahl (Ecuación~\ref{eq:amdahl}):
cuando $N$ tiende a infinito, $S(N)$ se acerca a $1/(1-p)$, y ya no importa
cuántos procesadores más se agreguen. Ese valor hay que sacarlo con la
fracción serial que de verdad se midió en T3 (el join), no con un número
inventado.

Esa fracción se calcula comparando el tiempo de T3 en pandas contra el
tiempo de T3 en PySpark con distintos executors (1, 2 y 4). Con el speedup
real $S_{obs}(N) = T_{pandas}/T_{spark}(N)$, se despeja $(1-p)$:

\begin{equation}
1 - p = \frac{\dfrac{1}{S_{obs}(N)} - \dfrac{1}{N}}{1 - \dfrac{1}{N}}
\label{eq:fraccion-serial}
\end{equation}
